...
public class HomeInventory extends JFrame{
    ...
    JButton previousButton = new JButton(new ImageIcon("prev.gif"));
    ...
    public HomeInventory(){
        ...
        inventoryToolBar.addSeperator();
        
        // previous button
        previousButton.setText("Previous");
        sizeButton(previousButton, bSize);
        previousButton.setToolTipText("Display Previous Item");
        previousButton.setHorizontalTextPosition(SwingConstants.BOTTOM);
        ...
        // store/ website from where item was purchased LabelText
        ...
        int x =0, y=0;
        //create and positioning 26 buttons
        for(int i=0; i<26; i++){
            //creates new button
    ...
    private void saveButtonActionPerformed(ActionEvent e){
        ...
        //moves all entries below new value down one position unless at end
        if(currentEntry!= numberEntries){
            for(int i =numberEntries; i>=currentEntry+1; i--){
                myInventory[i-1] = myInventory[i-2];
                myInventory[i-2] = myInventoryItem();
            }
        }
    ...
    private void photoButtonActionPerformed(ActionEvent e){
        ...
        openChooser.addChoosableFileFilter(new FileNameExtensionFile("Photo Files","jpg"));
        if(openChooser.showOpenDialog(this)==JFileChooser.APPROVE_OPTION)
            showPhoto(openChooser.getSelectedFiles().toString());
    }
    ...
    private void locationComboBoxActionPerformed(ActionEvent e){
        //if exist in list execute exit method
        if(locationComboBox.getItemCount()!=0){
            for(int i =0; i< locationComboBox.getItemCount(); i++){
    ...
    //If doesn't exists in the list, add to list box
    ...
    private void priceTextFieldAActionPerformed(ActionEvent e){
    ...
    private void checkSave(){
        ...
        if(!myInventory[currentEntry -1].description.equals(itemTextField.getText()))
            edited =true;
        else if(!myInventory[currentEntry -1].location.equals(locationComboBox.getAccessibleContext().getSelectedItem().toString()))
            edited = true;
...
class InventoryDocument implements Printable
{
    public int print(Graphics g, PageFormat pf, int pageIndex)
    {
        ...
        for (i = 0 + HomeInventory.entriesPerPage * pageIndex; i < iEnd; i++)
        {
            ...
            try
            {
                Image inventoryImage = new ImageIcon(HomeInventory.myInventory[i]).getImage();
